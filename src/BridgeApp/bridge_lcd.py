...
def draw_status(data_map):
    ...
    draw.text(
        (10, 50), f"System: {data_map.get('STATUS', 'N/A')}", fill=(255, 255, 255)
    )
    draw.text(
        (10, 80), f"Internet: {data_map.get('NET', 'Checking...')}", fill=(0, 255, 0)
    )
    draw.text(
        (10, 110),
        f"Mainframe: {data_map.get('MAINFRAME', 'Offline')}",
        fill=(255, 255, 0),
    )
